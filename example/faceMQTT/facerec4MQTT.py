...
username = 'user'
password = '1234'
deviceName = 'cameraFaceRec'

def on_connect(client, userdata, flags, rc):
    print("Connected with result code "+str(rc))
def on_message(client, userdata, msg):
    print(msg.topic,msg.payload.decode("utf-8"))
    if msg.topic == 'netpie/Inroom':
        global motion_flag,motion_time
        motion_time = idle_time
        motion_flag = True
        sentImg(None)
def sentImg (name = 'unknown'):
    retval, buffer = cv2.imencode('.jpg', frame)
    jpg_as_text = base64.b64encode(buffer)
    requests.post("http://192.168.4.1:3001/sensors", data={'name': deviceName, 'namePeople': name, 'img': jpg_as_text})                     
def motion_detection (Frame):
    Gray = cv2.cvtColor(Frame, cv2.COLOR_BGR2GRAY)
    #ret,black = cv2.threshold(Gray,40,100,cv2.THRESH_BINARY)
...
camera.crop = (0.23, 0.10, 0.41, 0.41)
camera.rotation = 180
camera.hflip = False
            ...
            for face_encoding in face_encodings:
                ...
                if True in matches:
                    ...
                    if motion_flag:
                        ...
                        local.publish("namePeople",name,'LOCAL')
                        namePre = name
                #client.publish('namePeople', payload=name, qos=0, retain=True)
